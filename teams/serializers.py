...
class AgencyUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Team
        fields = ("agency_id", "name", "amadeus_branded_fares", "api_username", "api_password", "style_group", "is_iframe",
                  "student_and_youth", "common_parameter_id", "admin_id")
    ...
    def validate(self, attrs):
        ...
        if not agency_id:
            raise CustomException(code=10, message=self.error_messages['invalid_agency'])
        if not name:
            raise CustomException(code=11, message=self.error_messages['invalid_name'])
        if not amadeus_branded_fares:
            raise CustomException(code=12, message=self.error_messages['invalid_amadeus_branded_fares'])
        if not api_username:
            raise CustomException(code=13, message=self.error_messages['invalid_api_username'])
        if not api_password:
            raise CustomException(code=14, message=self.error_messages['invalid_api_password'])
        if not style_group:
            raise CustomException(code=15, message=self.error_messages['invalid_style_group'])
        if not is_iframe:
            raise CustomException(code=16, message=self.error_messages['invalid_is_iframe'])
        if not student_and_youth:
            raise CustomException(code=17, message=self.error_messages['invalid_student_and_youth'])
        if not common_parameter_id:
            raise CustomException(code=18, message=self.error_messages['invalid_common_parameter'])
        if not admin_id:
            raise CustomException(code=19, message=self.error_messages['invalid_admin'])

        try:
            agency = Agency.objects.get(id=admin_id)
            admin = User.objects.get(id=admin_id)
            common_parameter = CommonParameters.objects.get(id=common_parameter_id)
            attrs['agency'] = agency
            attrs['admin'] = admin
